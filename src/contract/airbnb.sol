...
contract airbnb is ReentrancyGuard {
    ...
    mapping(address=>mapping(uint256 => string[])) books;
    ...
    */
    function checkBookings(uint256 id, string[] memory newBookings) public view returns(bool allFree , string[] memory bookedDates){
        ...
        if (arrayLength!=0){
            ...
            return (false , bookedDates);         
            
        } else{
            string[] memory emptyArray = new string[](0);
            return (true , emptyArray);
        }
    }
    ...
    function addDatesBooked(uint256 id , string[] memory newBookings) public nonReentrant payable{
        ...
        require(msg.value == rentals[id].pricePerDay  * newBookings.length ,
        "Please submit the asking price in order to complete the purchase");

        for(uint i =0 ; i<newBookings.length ; i++){
            rentals[id].datesBooked.push(newBookings[i]);
            books[msg.sender][id].push(newBookings[i]);
        }
    ...
    function getRental(uint256 id) public view returns(
        string memory , 
        string memory ,
        string memory ,
        string memory ,
    ...
    function getAllRentals() public view returns(rentalInfo[] memory){
        rentalInfo[] memory _rentals = new rentalInfo[](counter);
        for(uint i=0 ; i<counter ; i++){
            _rentals[i] = rentals[i]; 
        }
        return _rentals;
    }
